...
import { useCreatePaymentUrlMutation } from "src/redux/features/order/orderApi";
import {
  useAddOrderMutation,
...
const useCheckoutSubmit = () => {
  ...
  useEffect(() => {
    ...
    setCartTotal(total + shippingCost - discount);
  }, [
    total,
    shippingCost,
    discountPercentage,
  ...
  const handleCouponCode = (e) => {
    ...
    if (isError) {
      return notifyError("Có lỗi xảy ra");
    }
    ...
    if (dayjs().isAfter(dayjs(result[0]?.endTime))) {
      notifyError("Mã giảm giá hết hạn!");
      return;
    }

    if (total < result[0]?.minimumAmount) {
      notifyError(
        `Đơn hàng tôi thiểu là ${result[0].minimumAmount}đ !`
      );
      return;
    } else {
      notifySuccess(
        `Mã giảm giá ${result[0].title} đã được áp dụng ${result[0].productType}!`
      );
  ...
  const submitHandler = async (data) => {
    ...
    // get localStorage paymentMethod
